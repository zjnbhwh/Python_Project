...
class XtMail(ABCRelease.ABCRelease):
    def __init__(self):
        cf = configparser.ConfigParser()
        cf.read("../resource/system.ini")
        #本地编译后的class路径
        self.class_dir = cf.get(r"%s" % "xtmail", "class_dir")
        super(XtMail, self).__init__("xtmail")
    #该方法目前只处理了新增或修改的发布
...
if __name__ == "__main__":
    xtmail = XtMail()
    xtmail.logger.info("####################开始########################" + xtmail.date_file)
    xtmail.update()
    min = "2254"
    max = "2260"
    xtmail.logger.info("始版本号：" + min)
    xtmail.logger.info("终止版本号：" + max)
    # 获得svn文件,并返回存放路径
    #svnPath = mi.get_diff_file(min, max)

    xtmail.logger.info("####################结束#########################" + xtmail.date_file)
